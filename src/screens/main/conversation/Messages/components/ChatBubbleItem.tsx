...
import {CustomToastBottom, formatTime} from '../../../../../utils';
import {
  ComicService,
  MessageI,
  MessageType,
...
export const ChatBubble = React.memo((props: MessageI) => {
  ...
  const handleLinkPress = async () => {
    if (props.type === MessageType.LINK) {
      const url = props.message;
      if (url.includes('comicverse2.page.link/V9Hh/comicdetail')) {
        const uuid = url.split('comic_uuid=')[1];
      ...
      <View>
        {!isUser ? (
          <View style={[styles.viewRow, {alignItems: 'flex-start'}]}>
            <View>
              <Image style={styles.imageNoteUser} source={{uri: imageSource}} />
            </View>
            <View>
              <View style={styles.bubbleNotUser}>
                <Text
                  style={[
                    styles.textNotUser,
                    props.type === MessageType.LINK
                      ? styles.styleTextLink
                      : null,
                  ]}
          ...
          <View>
            <View style={styles.bubbleUser}>
              <Text
                style={[
                  styles.textUser,
                  props.type === MessageType.LINK ? styles.styleTextLink : null,
                ]}
                onPress={handleLinkPress}>
                {props.message}
              </Text>
